...
export const ShaderCanvas: React.FC<ShaderCanvasProps> = React.memo(({ fragmentSrc, onError, uniforms, cameraRef, isHdEnabled, isFpsEnabled, isPlaying, shouldReduceQuality }) => {
  ...
  const uniformKeys = Object.keys(uniforms).join(',');
  ...
  useEffect(() => {
    ...
    const fragmentTemplate = `#version 300 es
      ...
      void main() {
        ...
        // Fallback: if color is still pure black, show a debug magenta
        // This helps identify if the shader code failed to assign to 'o'
        if (length(o.rgb) < 0.001) {
            o = vec4(1.0, 0.0, 1.0, 1.0);
        }
    ...
    const render = (timestamp: number) => {
      if (lastTimestamp === 0) {
          ...
          console.log('ShaderCanvas: First frame rendering', {
              width: canvas.width,
      ...
      gl.clearColor(1.0, 0.0, 0.0, 1.0); // Debug Red: if you see red, the pipeline is working but shader is failing
  ...
  }, [fragmentSrc, onError, cameraRef, uniformKeys]);
