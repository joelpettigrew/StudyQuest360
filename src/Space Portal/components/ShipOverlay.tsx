...
// Flying Book Ship Body
float sdFractalShip(vec3 p) {
    p /= u_generalScale;
    
    // The Spine
    float spine = length(max(abs(p) - vec3(0.05, 0.05, 0.8), 0.0)) - 0.02;
    
    // The Pages (Open Book)
    vec3 pPage = p;
    float angle = 0.3; // Slight V-shape
    pPage.x = abs(pPage.x) - 0.05;
    pPage.xy *= rot(angle * sign(p.x));
    float pages = length(max(abs(pPage) - vec3(0.6, 0.02, 0.75), 0.0)) - 0.01;
    
    // Stylized "Wing" Pages (Outer edges)
    vec3 pWing = p;
    pWing.x = abs(pWing.x) - 0.6;
    pWing.xy *= rot(-0.5 * sign(p.x));
    float wings = length(max(abs(pWing) - vec3(0.4, 0.01, 0.6), 0.0)) - 0.01;
    
    return min(min(spine, pages), wings) * u_generalScale;
}
...
export const ShipOverlay: React.FC = () => {
    const { viewMode, pressedKeys, effectiveShipConfigRef, cameraAngularVelocityRef, shipConfig } = useAppContext();
    ...
    const compile = (type: number, src: string) => {
        ...
        if (!gl.getShaderParameter(s, gl.COMPILE_STATUS)) {
            ...
            console.error("ShipOverlay Shader Error:", err);
        }
        return s;
    };
    ...
    if (!gl.getProgramParameter(p, gl.LINK_STATUS)) {
        console.error("ShipOverlay Program Link Error:", gl.getProgramInfoLog(p));
    }
    gl.useProgram(p);
        ...
        const render = (t: number) => {
            ...
            const isThrustingNow = pressedKeysRef.current.has(' ');
            const isBraking = false; // Simplified, no braking
